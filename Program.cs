        ...
        */
        ...
        public static string MostCommonWord(string paragraph, string[] banned)
        {
            try
            {
                ...
                string temp = "";
                //loop to iterate through the characters of the paragraph
                for (int i = 0; i < paragraph.Length; i++)
                {
                    // condition for ingnoring the punctuation                  
                    if (paragraph[i] == ',') continue;
                    //condition to check  whether we got a word in temp 
                    if (paragraph[i] == ' ' || paragraph[i] == '.')
                    {
                        string new_string = temp.ToLower();
                        if (map.ContainsKey(new_string))
                        {
                            //ncrementing the value if key is found in th map            
                            map[new_string] = map[new_string] + 1;
                        }
                        else
                        {
                            //insert a new map value
                            map[new_string] = 1;
                        }
                        temp = "";
                    }
                ...
                {
                    if (map.ContainsKey(toSmall(banned[i])))
                    {
                        map[toSmall(banned[i])] = 0;
                    }
                }
                var value = map.Keys.ToList();
                int maxRecent = 0;
                string ans = "";
                //iterating to find the most recent character
                foreach (var key in value)
                {
                    if (map[key] > maxRecent)
                    {
                        maxRecent = map[key];
                        ans = key;
                    }
        ...
        public static int FindLucky(int[] arr)
        {
            try
            {
                //defining dictionary as luckynumber
                Dictionary<int, int> luckyNumber = new Dictionary<int, int>();
                //iterating through input  arr for mapping frequency and its value
                foreach (var ele in arr)
                {
                    // condition to check whether                          
                    if (luckyNumber.ContainsKey(ele))
                    {
                        luckyNumber[ele] = luckyNumber[ele] + 1;
                    }
                    else
                    {
                        luckyNumber[ele] = 1;
                ...
                var val = luckyNumber.Keys.ToList();
                int ans = -1;
        ...
        */
        ...
        public static string GetHint(string secret, string guess)
        {
            try
            {
                int x = 0;
                int y = 0;
                bool[] sameValue = new bool[secret.Length];
                Dictionary<char, int> map = new Dictionary<char, int>();
                // loop for iterating through  secret string
                for (int i = 0; i < secret.Length; i++)
                {
                    if (secret[i] == guess[i])
                    {
                        // if secret char and guess char matches then incrementing value 
                        x++;
                        sameValue[i] = true;
                    ...
                    {
                        //setting false to revisit again in next loop
                        sameValue[i] = false;
                        if (map.ContainsKey(secret[i]))
                        {
                            //inserting map values and keys                   
                ...
                {
                    //skipping for the value that is set to true
                    if (sameValue[i]) continue;
                ...
                //adding string for answers matching
                string ans = x + "A" + y + "B";
                return ans;
            }
            catch (Exception)
        ...
        */
        ...
        public static List<int> NumberOfLines(int[] widths, string s)
        {
            try
            {
                ...
                {
                    temp = widths[_c - 97];
                    // if sum + temp value is greater than 100
                    if (sum + temp > 100)
                    {
                        row++;
                        sum = temp;
